class BibliographicReferencesController < ApplicationController
  ...
  def index
    @bibliographic_references = if params[:query].present?
                                  if params[:exact] == "true"
                                    BibliographicReference.where("LOWER(title) = LOWER(?)", params[:query])
                                  else
    ...
    begin
      ...
      if response.code == "200"
        ...
        if data && data["message"]
          ...
          year = nil
          year = message["published"]["date-parts"].first.first if message["published"] && message["published"]["date-parts"] && message["published"]["date-parts"].first
  ...
  def create
    ...
    # Verificar se já existe uma referência com o mesmo título
    existing_reference = BibliographicReference.where("LOWER(title) = LOWER(?)", @bibliographic_reference.title).first

    respond_to do |format|
      if existing_reference
        format.html { redirect_to new_bibliographic_reference_path, alert: "Já existe uma referência bibliográfica com este título." }
        format.json { render json: { errors: { title: ["já existe uma referência bibliográfica com este título"] } }, status: :unprocessable_entity }
      elsif @bibliographic_reference.save
        ...
        format.json { render json: { errors: @bibliographic_reference.errors }, status: :unprocessable_entity }
      end
    end
  end
  ...
  def update
    ...
    # Verifica se existe outra referência com o mesmo título (que não seja essa que estamos atualizando)
    existing_reference = BibliographicReference.where("LOWER(title) = LOWER(?) AND id != ?", bibliographic_reference_params[:title], @bibliographic_reference.id).first

    respond_to do |format|
      if existing_reference
        format.html { render :edit, status: :unprocessable_entity, alert: "Já existe uma referência bibliográfica com este título." }
        format.json { render json: { errors: { title: ["já existe uma referência bibliográfica com este título"] } }, status: :unprocessable_entity }
      elsif @bibliographic_reference.update(bibliographic_reference_params)
